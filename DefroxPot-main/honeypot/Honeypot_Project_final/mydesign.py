...
# BUG FIX #7: Removed mutable global dict — now created per-call in track_and_response()


def track_and_response(request, page):
    # BUG FIX #7: Create a fresh dict per request (was module-level mutable global)
    json_logs = {
        "ip_addr": "",
        "date": "",
        "timestamp": "",
...
# SQL lite3
# BUG FIX #8: Removed module-level sqlite3.connect() — it can't be shared across threads.
# Now using lazy initialization via get_db() and init_db().
...
# Run init on import (safe — creates only if not exists)
try:
    init_db()
except Exception:
    pass
